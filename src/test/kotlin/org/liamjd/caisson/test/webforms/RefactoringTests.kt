...
class NewTests : Spek({
	...
	describe("Uploading files") {
		...
		it("Uploading two documents, each with their own input name") {
			...
			every { doc1Part.name} returns uploadPhoto
			...
			every { doc2Part.name} returns uploadPhoto
			...
			val legalDocuments = NewWebForm(mSparkRequest,LegalDocuments::class, arrayListOf(uploadDoc1,uploadDoc2)).get() as LegalDocuments
